...
Aplicación real, incremental, mobile-first. Fases 1–5.1 conservadas. Fase 6 autorizada expresamente: chat, notificaciones, recordatorios, email/push, PWA, jobs y hardening final, según docs/phase-6-requirements.md. No ampliar ese alcance. La especificación íntegra y vinculante está en `docs/product-requirements.md`; no eliminar ni simplificar requisitos futuros.
...
Horas locales convertidas en SQL usando homes.timezone, guardadas como timestamptz. Rechazar horas DST inexistentes; repetidas usan interpretación estándar de PostgreSQL. Editar horas sin cambio conserva el instante original; formularios con timezone obsoleta se rechazan. Creador de actividad queda apuntado; inscripción única/idempotente, edición mantiene participantes. CalendarEvent es una capa de lectura, nunca otra tabla ni disparador de generación de tareas/gastos. Mes + agenda, tipos y solo lo mío, enlaces al módulo fuente. Realtime INSERT/UPDATE por home_id para las cuatro tablas. Detalles y límites en docs/phase-4-delivery.md. La Fase 5 añade puntos/castigos/ranking sin alterar estos contratos; chat y notificaciones continúan pendientes.

## Convivencia — Fase 5

Migraciones aditivas 007, 008 y 009; conservar 001–008 ya aplicadas. `src/features/community` implementa puntos/castigos/ranking/motivos; actividades continúa en `/community/activities`. Cuatro tablas públicas sin autor/editor de valoraciones y cinco privadas con RLS; `community_private` sin privilegios cliente ni publicación Realtime. Solo `rating_author_labels` devuelve identidad al propio autor o en valoraciones identificadas; no crear joins públicos al autor anónimo. Fotos privadas con ruta aleatoria, propietario/metadata de Storage saneados y recodificación WebP sin EXIF; límite 5 MiB/20 MP. Servir con comprobación de membresía por petición, no-store y descarga nueva: una copia anterior en CDN no equivale a acceso nuevo autorizado ni puede prometerse su revocación inmediata.

RPCs bajo bloqueo del piso, versiones y UUID de creación. Valoraciones manuales editables por activos, receptor inmutable; baja lógica, auditoría privada, no editar/borrar automáticas. Tres positivos vivos se consumen en orden fecha/UUID, compensando el negativo más antiguo; sin negativos generan crédito pendiente. Correcciones revocan/reconstruyen asignaciones conservando auditoría. Castigos únicos piso/persona/umbral de 5; nunca se retiran al bajar puntos ni se duplican al recuperar un umbral; completar no gasta negativos.

Retrasos: bloques completos de 24 h desde el deadline absoluto, hasta completed_at; responsable según historial en cada frontera, sin bloqueados/cancelados/inactivos. 008 inicia nueva elegibilidad al reingresar, sin retroactividad sobre el periodo fuera. 009: leave_home comprueba saldo, fija un único now() de transacción, agota todos los lotes de retrasos y reconcilia castigos mientras el usuario sigue activo; después desactiva y conserva reconcile_departure_chores. left_at es ese mismo cutoff. Salida atómica incluso con más de 500 decisiones; un error revierte todo. No reescribir sanciones emitidas; instancia/día único. Reconciliar solo en Convivencia (excepto Actividades), Tareas y completado, máximo 500 días procesados por llamada habitual y continuación sin límite total; la salida agota los lotes antes de desactivar. Sin cron. Ranking solo activos, negativos ascendente/positivos disponibles descendente/UUID. Inicio solo resumen propio. [Semántica y límites](docs/phase-5-delivery.md). Fase 6 implementa chat/notificaciones conservando anonimato y estos contratos.
...
Conservar 001–009. Nuevas migraciones aditivas; chat con autoría propia, bajas auditadas, adjuntos privados y cursor. Notificaciones canónicas por destinatario, sin autores de ratings en ningún canal. Service-role solo en worker servidor de entregas, nunca en flujo de usuario ni NEXT_PUBLIC. Credenciales email/push separadas y canales externos opt-in. Cron de producción independiente de npm dev. Service worker no cachea datos privados/auth ni simula escrituras offline.
